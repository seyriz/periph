...
// Package sh1106smoketest is leveraged by periph-smoketest to verify that two
// sh1106, one over I²C, one over SPI, can display the same output.
package sh1106smoketest
...
func (s *SmokeTest) Description() string {
	return "Tests sh1106 over I²C and SPI by displaying multiple patterns that exercises all code paths"
}
...
func (s *SmokeTest) run(i2cBus i2c.Bus, spiPort spi.PortCloser, dc gpio.PinOut, w, h int, rotated bool) (err error) {
	...
	start := time.Now()
	i2cDev, err2 := sh1106.NewI2C(i2cBus, w, h, rotated)
	s.timings[0] = time.Since(start)
	if err2 != nil {
		return err2
	}
	start = time.Now()
	spiDev, err2 := sh1106.NewSPI(spiPort, dc, w, h, rotated)
	s.timings[1] = time.Since(start)
	if err2 != nil {
		return err2
	}

	s.devices = []*sh1106.Dev{i2cDev, spiDev}
